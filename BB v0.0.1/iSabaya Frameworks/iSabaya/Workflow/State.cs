...
namespace iSabaya
{
	...
	public class State
	{
		...
		{
			if (title != null) title.Save(context);
			if (description != null) description.Save(context);
		...
		{
			ICriteria crit = context.PersistencySession.CreateCriteria(typeof(State));
		...
		*/
        ...
        {
            return (State)context.PersistencySession.Get(typeof(State), p);
        }
