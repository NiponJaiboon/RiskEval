...
namespace WebHelper.Controls
{
    ...
    public class MoneyControl : iSabayaWebControlBase
    {
        ...
        public Money Value
        {
            get
            {
                ...
                if (spnAmount.Value != null)
                    m.Amount = Convert.ToDecimal(spnAmount.Value);
                if (cboCurrency.SelectedItem != null)
                    m.CurrencyID = (int)cboCurrency.SelectedItem.Value;
                return m;
            }
            set
            {
                ...
                if (value != null)
                {
                    ...
                    item = cboCurrency.Items.FindByValue(value.CurrencyID);
                    if (item != null)
                        item.Selected = true;
                }
                else
                {
                    ...
                    if (iSabayaContext.imSabayaConfig.DefaultCurrency != null)
                    {
                        item = cboCurrency.Items.FindByValue(iSabayaContext.imSabayaConfig.DefaultCurrency.ID);
                        if (item != null)
                            item.Selected = true;
                    }
                    else
        ...
        private void InitializeControls()
        {
            ...
            {
                if (lowerBound.CurrencyID != upperBound.CurrencyID)
        ...
        protected override void OnLoad(EventArgs e)
        {
            base.OnLoad(e);
            InitializeControls();
            IEnumerable<Currency> currencies = Currency.List(iSabayaContext);
            ...
            if (!string.IsNullOrEmpty(FixCurrencyCode) || UpperBound != null || LowerBound != null)
            {
                ...
                {
                    if (c.Code == code)
                        cboCurrency.Items.Add(c.Code, c.ID);
                }
                cboCurrency.SelectedIndex = 0;
            ...
            else
            {
                foreach (Currency c in currencies)
                    cboCurrency.Items.Add(c.Code, c.ID);
                ListEditItem item = cboCurrency.Items.FindByValue(iSabayaContext.imSabayaConfig.DefaultCurrency.ID);
                if (item != null)
                    item.Selected = true;
                else
                    cboCurrency.SelectedIndex = 0;
