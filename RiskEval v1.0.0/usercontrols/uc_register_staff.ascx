<%@ Control Language="C#" AutoEventWireup="true" CodeFile="uc_register_staff.ascx.cs" Inherits="usercontrols_uc_register" %>
<script type="text/javascript">
    $(document).ready(function () {

        $("#<%= cbk_department_selectAll.ClientID %>").click(function () {
            $('#<%=pnl_department_list.ClientID%>').find('input:checkbox').attr('checked', this.checked);
        });

        $('#<%=pnl_department_list.ClientID%>').find('input:checkbox').each(function () {
            $(this).click(function () {
                var values = true;
                $('#<%=pnl_department_list.ClientID%>').find('input:checkbox').each(function () {
                ...
                $("#<%= cbk_department_selectAll.ClientID %>").attr('checked', values);
            })
        });
    });
</script>
    ...
    <table>
        ...
        <tr>
            ...
            <td colspan="3">
                ...
                <asp:RegularExpressionValidator ID="req_email" runat="server" 
                    ControlToValidate="txt_email" ErrorMessage="อีเมล์ไม่ถูกต้อง" 
        ...
        <tr>
            <td>
                สังกัด</td>
            <td colspan="3">
                <asp:DropDownList ID="ddl_office" runat="server" AppendDataBoundItems="True">
        ...
        <tr>
            ...
            <td colspan="3">
                ...
                <asp:RequiredFieldValidator ID="RequiredFieldValidator12" runat="server" 
                    ...
                    ValidationGroup="register" InitialValue="-1">*</asp:RequiredFieldValidator>
                <asp:DropDownList ID="ddl_rolesub" runat="server" DataSourceID="sds_rolesub" 
                    DataTextField="subrole_name" DataValueField="subrole_id" Visible="False">
                </asp:DropDownList>
            </td>
        </tr>
        <tr>
        ...
        <tr>
            <td>หน่วยงานที่ดูแล<asp:CustomValidator ID="CustomValidator_department" 
                    runat="server" ErrorMessage="กรุณาเลือกหน่วยงานที่ดูแล" 
                    onservervalidate="CustomValidator_department_ServerValidate" 
                    ValidationGroup="register">*</asp:CustomValidator>
            </td>
            <td colspan="3">
                <asp:CheckBox ID="cbk_department_selectAll" runat="server" 
                     Text="เลือกหน่วยงานทั้งหมด"
                     />
            </td>
        </tr>
        <tr>
            <td colspan="4">
                <asp:Panel ID="pnl_department_list" runat="server" style="width:100%">
                    <asp:CheckBoxList ID="cbklist_department" runat="server" 
                        RepeatColumns="2">
                    </asp:CheckBoxList>
                </asp:Panel>
            </td>
...
SET @Identity = @@Identity" 
    ...
    SelectCommand="SELECT p_id, p_idno, p_name_thai, p_sname_thai, p_name_eng, p_sname_eng, p_address, p_phone, p_phone_ext, p_mobile, m_id, o_id, p_is_active, p_is_delete, p_role_id,p_phone_direct,p_email FROM persons WHERE (p_idno = @p_idno) AND (p_is_delete &lt;&gt; 1)" 
    ...
    <InsertParameters>
        ...
        <asp:ControlParameter ControlID="ddl_role" Name="p_role_id" 
            PropertyName="SelectedValue" />
        <asp:ControlParameter ControlID="ddl_rolesub" Name="p_role_sub_id" 
            PropertyName="SelectedValue" />    
        <asp:Parameter Type="Int32" Direction="Output" Name="Identity" 
            DefaultValue="" />
    </InsertParameters>
    <SelectParameters>
        <asp:ControlParameter ControlID="txt_idno" Name="p_idno" PropertyName="Text" />
...
<asp:SqlDataSource ID="sds_register_staff_persons_detail" runat="server" 
    ConnectionString="<%$ ConnectionStrings:GovBudgetingConnectionString %>" 
    InsertCommand="INSERT INTO persons_detail(p_id, d_id, modified_date ) VALUES (@p_id, @d_id, GETDATE())" 
    
    SelectCommand="SELECT p_id, d_id FROM persons_detail WHERE (pdt_is_delete &lt;&gt; 1) AND (p_id = @p_id) " 
...
<asp:SqlDataSource ID="sds_role" runat="server" 
    ...
    SelectCommand="SELECT [role_id], [role_name] FROM [roles] WHERE ([role_name] LIKE '%' + @role_name + '%')">
    <SelectParameters>
        <asp:Parameter DefaultValue="(สำนักงบประมาณ)" Name="role_name" Type="String" />
    </SelectParameters>
</asp:SqlDataSource>


<asp:SqlDataSource ID="sds_rolesub" runat="server" 
    ConnectionString="<%$ ConnectionStrings:GovBudgetingConnectionString %>" 
    SelectCommand="SELECT subrole_id, subrole_name FROM roles_sub WHERE (role_id = @role_id)">
    <SelectParameters>
        <asp:ControlParameter ControlID="ddl_role" Name="role_id" 
            PropertyName="SelectedValue" />
    </SelectParameters>
</asp:SqlDataSource>
